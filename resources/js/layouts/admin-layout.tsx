...
export default function AdminLayout({ children, title, breadcrumbs = [] }: AdminLayoutProps) {
    ...
    return (
        <div className="flex min-h-screen flex-col bg-[#f4f6f9] font-sans text-slate-800 antialiased dark:bg-slate-950 dark:text-slate-100 print:block print:min-h-0 print:bg-white print:text-slate-900">
            ...
            <div className="relative flex flex-1 overflow-hidden print:block print:overflow-visible">
                ...
                {/* Mobile Backdrop */}
                {isMobileMenuOpen && (
                    <div className="fixed inset-0 z-30 bg-black/50 lg:hidden print:hidden" onClick={() => setIsMobileMenuOpen(false)} />
                )}

                {/* Content Wrapper (AdminLTE .content-wrapper) */}
                <div className="flex min-w-0 flex-1 flex-col overflow-y-auto pb-16 lg:pb-0 print:block print:w-full print:overflow-visible print:bg-white print:pb-0">
                    {/* Content Header */}
                    <div className="border-b border-slate-200/80 bg-white px-4 py-3 sm:px-6 sm:py-4 dark:border-slate-800 dark:bg-slate-900/50 print:hidden">
                        <div className="flex flex-col gap-1.5 sm:flex-row sm:items-center sm:justify-between">
                            <h1 className="text-lg font-bold tracking-tight text-slate-900 sm:text-xl dark:text-slate-100">{title || 'Dashboard'}</h1>
                    ...
                    {/* Flash Toast Alerts */}
                    {flash?.success && (
                        <div className="mx-4 mt-3 flex items-center gap-2.5 rounded-md border border-emerald-200 bg-emerald-50 px-4 py-3 text-xs text-emerald-800 shadow-sm sm:mx-6 sm:mt-4 dark:border-emerald-900/50 dark:bg-emerald-950/40 dark:text-emerald-300 print:hidden">
                            <CheckCircle2 size={16} className="shrink-0 text-emerald-600 dark:text-emerald-400" />
                            <span>{flash.success}</span>
                        </div>
                    )}
                    {flash?.error && (
                        <div className="mx-4 mt-3 flex items-center gap-2.5 rounded-md border border-rose-200 bg-rose-50 px-4 py-3 text-xs text-rose-800 shadow-sm sm:mx-6 sm:mt-4 dark:border-rose-900/50 dark:bg-rose-950/40 dark:text-rose-300 print:hidden">
                    ...
                    {/* Main Page Body */}
                    <main className="flex-1 p-3 sm:p-6 print:m-0 print:block print:w-full print:overflow-visible print:bg-white print:p-0">
                        {children}
                    </main>

                    {/* AdminLTE Footer */}
                    <footer className="flex flex-col items-center justify-between gap-2 border-t border-slate-200 bg-white px-4 py-3 text-xs text-slate-500 sm:flex-row sm:px-6 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400 print:hidden">
                        <div>
                            <strong>
                                Copyright &copy; 2026{' '}
